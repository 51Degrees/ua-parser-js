...
const UAParser = (key, headers) => {
  ...
  if (Object.keys(headers).some(k => typeof headers[k] !== "string")) throw Error(ECustomError.INVALID_HEADERS_VALUE);
  if (!Object.keys(headers).some(k => k.toLowerCase() === "user-agent" || k.toLowerCase().includes("sec-ch"))) throw Error(ECustomError.INCOMPLETE_HEADER_MAP);
